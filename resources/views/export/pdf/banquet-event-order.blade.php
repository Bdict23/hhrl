...
<body>
    <div class="container">
        ...
        <div class="head avoid-break">
            <div style="text-align: center;">
                @if (auth()->user()->branch->company->company_logo)
                    <img src="{{ public_path('images/' . auth()->user()->branch->company->company_logo) }}"
                        alt="Branch Logo">
                @endif
        ...
        <!-- Title -->
        <div class="section-title">Banquet Event Order &nbsp;&nbsp;<strong>#{{ $eventDetails->reference }}</strong>
        </div>
            ...
            <table class="bordered info-table avoid-break">
                <tbody>
                    <tr class="row-group">
                        ...
                        <td style="width: 50%;"><strong>Customer Name:</strong>
                            <br>{{ $eventDetails->customer->customer_fname ?? '' }}
                            {{ $eventDetails->customer->customer_lname ?? '' }}
                            {{ $eventDetails->customer->suffix ?? '' }}
                        </td>
                    </tr>
                    <tr class="row-group">
                        <td><strong>Event Date:</strong>
                            <br>{{ \Carbon\Carbon::parse($eventDetails->start_date)->format('M. d') }} -
                            {{ \Carbon\Carbon::parse($eventDetails->end_date)->format('d, Y') }}
                        </td>
                    ...
                    <tr class="row-group">
                        ...
                        <td><strong>Contact #:</strong>
                            <br>{{ $eventDetails->customer->contact_no_1 ?? '' }}{{ $eventDetails->customer->contact_no_2 ?? false ? ' / ' . $eventDetails->customer->contact_no_2 : '' }}
                        </td>
                    </tr>
                    <tr class="row-group">
                    ...
                    <tr class="row-group">
                        <td><strong>Start Time:</strong>
                            <br>{{ \Carbon\Carbon::parse($eventDetails->arrival_time)->format('h:i A') }}<br>({{ \Carbon\Carbon::parse($eventDetails->start_date)->format('M. d, Y') }})
                        </td>
                        <td><strong>End Time:</strong>
                            <br>{{ \Carbon\Carbon::parse($eventDetails->departure_time)->format('h:i A') }}<br>({{ \Carbon\Carbon::parse($eventDetails->end_date)->format('M. d, Y') }})
                        </td>
                    </tr>
                </tbody>
            </table>
                    ...
                    @forelse($eventDetails->eventMenus ?? [] as $eventMenu)
                        <tr class="row-group">
                            ...
                            <td class="text-right currency">
                                &#8369;{{ number_format($eventMenu->price->amount ?? 0 * ($eventMenu->qty ?? 0), 2) }}
                            </td>
                        </tr>
                    @empty
                        <tr>
                ...
                <tbody>
                    @forelse($eventDetails->eventVenues ?? [] as $eventVenue)
                        <tr class="row-group">
                            ...
                            <td class="text-sm text-center">{{ $eventVenue->venue->capacity ?? '-' }} Guests</td>
                            <td class="text-center">{{ $eventVenue->qty ?? '0' }}</td>
                            <td class="text-right currency">
                                &#8369;{{ number_format($eventVenue->ratePrice->amount * ($eventVenue->qty ?? 0), 2) }}
                            </td>
                        </tr>
                    @empty
                        <tr>
                ...
                <tbody>
                    @forelse($eventDetails->eventServices ?? [] as $eventService)
                        <tr class="row-group">
                            ...
                            <td class="text-right currency">
                                &#8369;{{ number_format($eventService->price->amount ?? 0 * ($eventService->qty ?? 0), 2) }}
                            </td>
                        </tr>
                    @empty
                        <tr>
        ...
        <div class="totals-section avoid-break">
            ...
            <div class="totals-row grand-total">
                ...
                <span class="totals-amount currency"
                    style="font-size: 12px;">&#8369;{{ number_format($eventDetails->total_amount, 2) }}</span>
            </div>
        </div>

        <!-- Signature Section -->
        <div class="signature-section avoid-break">
            <div style="text-align: center; margin-bottom: 15px; font-weight: 600; font-size: 11px; color: #2c3e50;">
                Authorizations & Signatures</div>
            <div class="signature-row">
                <div class="signature-col">
                    <div class="signature-label">Prepared By:</div>
                    <div class="signature-line"></div>
                    <p class="signature-name">{{ $eventDetails->createdBy->name ?? '' }}
                        {{ $eventDetails->createdBy->last_name ?? '' }}</p>
                    <p class="signature-title">{{ $eventDetails->createdBy->position->position_name ?? '' }}</p>
                    <p class="text-xs text-muted">
                        {{ \Carbon\Carbon::parse($eventDetails->created_at)->format('M. d, Y') }}</p>
                </div>
                <div class="signature-col">
                    <div class="signature-label">Reviewed By:</div>
                    <div class="signature-line"></div>
                    <p class="signature-name">{{ $eventDetails->reviewer->name ?? '' }}
                ...
                <div class="signature-col">
                    ...
                    <div class="signature-line"></div>
                    <p class="signature-name">{{ $eventDetails->approver->name ?? '' }}
        ...
        <!-- Footer -->
        <div
            style="margin-top: 20px; padding-top: 10px; border-top: 1px solid #bdc3c7; text-align: center; font-size: 11px; color: #95a5a6;">
