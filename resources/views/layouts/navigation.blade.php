                    ...
                    <x-dropdown>
                        ...
                        <x-slot name="content">
                            <x-dropdown-link :href="url('/cardex')" class="no-underline" data-bs-toggle="modal"
                                onclick="unhideModal()" data-bs-target="#cardexModal">
                                {{ __('Cardex') }}
                            </x-dropdown-link>
                    ...
                    <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex" style="margin-top: 23px">
                        <x-dropdown>
                            ...
                            <x-slot name="content">
                                ...
                                @endif
                                @if (auth()->user()->employee->getModulePermission('Accounting - Template Approval') == 1)
                                    <x-dropdown-link :href="url('/template-approval-summary')" class="no-underline" style="font-size: smaller">
                                        {{ __('Template - Approval') }}
                                    </x-dropdown-link>
                                @endif
                    ...
                    <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex" style="margin-top: 23px">
                        <x-dropdown>
                            ...
                            <x-slot name="content">
                                ...
                                @endif
                                @if (auth()->user()->employee->getModulePermission('Restaurant - Order Billing') != 2)
                ...
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        ...
                        @if (auth()->user()->employee->getModulePermission('Restaurant - Order Billing') == 1 &&
                                auth()->user()->employee->hasOpenShift() == false)
                            <x-dropdown-link :href="url('/make-open-shift')" class="no-underline">
                                {{ __('Open Cashier Shift') }} &nbsp;<i class="bi bi-box-seam"></i>
                            </x-dropdown-link>
                        @endif
                        @if (auth()->user()->employee->hasOpenShift() &&
                                auth()->user()->employee->getModulePermission('Restaurant - Order Billing') == 1)
                            <x-dropdown-link :href="url(
                                '/close-cashier-shift?shift=' . auth()->user()->employee->id . '&referenced=current',
                            )" class="no-underline">
        ...
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if (auth()->user()->employee->getGroupedModulePermissions('Master Data') != 2 ||
                ...
                <div x-data="{ open: false }">
                    <button @click="open = !open"
                        class="w-full px-4 py-2 text-sm font-medium text-left text-gray-500 transition duration-150 ease-in-out hover:text-gray-700 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 focus:text-gray-700">
                        {{ __('Tools') }}
                    </button>
            ...
            <div x-data="{ open: false }">
                ...
                <div x-show="open" class="pl-4 space-y-1">
                    <x-responsive-nav-link class="no-underline" class="no-underline" data-bs-toggle="modal"
                        onclick="unhideModal()" data-bs-target="#cardexModal">
                        {{ __('Cardex') }}
                    </x-responsive-nav-link>
                    @if (auth()->user()->employee->getModulePermission('Back Orders') != 2)
