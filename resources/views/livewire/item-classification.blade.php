<div>
                ...
                <div class="col-md-6">
                    @if (auth()->user()->employee->getModulePermission('Item Classifications') == 1)
                        <x-primary-button type="button" class="mb-3"
                            onclick="showTab('classification-form-create', document.querySelector('.nav-link.active'))">+
                            Add
                            Classification</x-primary-button>
                    @endif

                    <x-secondary-button type="button" class="mb-3" wire:click="fetchData()"> <span
                            wire:loading.remove wire:target="fetchData()">Refresh</span>
                        <span wire:loading wire:target="fetchData()"><span class="spinner-grow spinner-grow-sm"></span>
                            Wait..</span></x-secondary-button>
                </div>
                <div class="col-md-6">
                    <div class="mb-3 input-group">
                        <span class="input-group-text">Search</span>
                        <input type="text" class="form-control" id="classification-search"
                            onkeyup="filterClassifications()">
                        <script>
                            ...
                            @forelse ($classifications ?? [] as $classification)
                                <tr>
                                    ...
                                    <td
                                        ...
                                        {{ $classification->classification_description }}</td>
                                    ...
                                    @if (auth()->user()->employee->getModulePermission('Item Classifications') == 1)
                                        <td class="text-end">
                                            <a href="#" class="btn btn-sm btn-primary" data-bs-toggle="modal"
                                                data-bs-target="#classificationUpdate"
                                                onclick="updateClassification({{ json_encode($classification) }})"
                                                wire:click="editClassification({{ $classification->id }})">Edit</a>
                                            <a href="#" class="btn btn-sm btn-danger"
                                                wire:click="deactivate({{ $classification->id }})">Delete</a>
                                        </td>
                                    @endif
                                </tr>
                            @empty
    ...
    <div id="classification-form-create" class="tab-content card" style="display: none" wire:ignore.self>
        ...
        <div class="card-body">
            ...
            <form wire:submit.prevent="store">
                ...
                <div class="mb-3">
                    <label for="classification_description-input" class="form-label">Description <span
                            style="color: red;">*</span></label>
                    <textarea class="form-control" id="classification_description-input" wire:model="classification_description"
                        rows="3"></textarea>
    ...
    <div id="classificationUpdate" class="modal fade" tabindex="-1" aria-labelledby="updateClassificationModal"
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="classification_name-update_input" class="form-label">Classification Name <span
                                style="color: red;">*</span></label>
                        <input type="text" class="form-control" id="classification_name-update_input"
                            wire:model="classification_name">
                        @error('classification_name')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="mb-3">
                        <label for="classification_description-update_input" class="form-label">Description <span
                                style="color: red;">*</span></label>
                        <textarea class="form-control" id="classification_description-update_input" wire:model="classification_description"
                            rows="3"></textarea>
                        @error('classification_description')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
