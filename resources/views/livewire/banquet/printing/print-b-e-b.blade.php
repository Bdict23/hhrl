<div>
   <div class="mb-4 head">
    ...
    <strong class="mt-6 mb-4">Banquet Event Budget - {{ $banquetEventBudget->reference_number }}</strong>
        ...
        <tbody>
            ...
            <tr>
                <td><strong class="m-1">Start Date:</strong> {{ (\Carbon\Carbon::parse($banquetEventBudget->event->start_date)->format('M. d-'))}}{{ (\Carbon\Carbon::parse($banquetEventBudget->event->end_date)->format('d, Y')) }}</td>
    ...
    <table class="w-full mt-2 table-bordered">
            ...
            <tbody>
                @foreach($banquetEventBudget->event->eventMenus ?? [] as $eventMenu)
                    <tr>
                        <td>
                                <span class="ml-1 break-all t-sm">{!! ($eventMenu->menu->menu_name) ? $eventMenu->qty . 'x '.$eventMenu->menu->menu_name . ' - ₱' . number_format($eventMenu->price->amount * $eventMenu->qty, 2) : '' !!}</span>
                        </td>
                    </tr>
                 @endforeach
            </tbody>
