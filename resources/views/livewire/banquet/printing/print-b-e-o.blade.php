<div>
    ...
    <strong class="mb-4 mt-6">Banquet Event Order - {{ $eventDetails->reference }}</strong>
    <table class="w-full table-bordered mt-2">
        <tbody>
            <tr>
                <td><strong class="m-1">Event Name:</strong> {{ $eventDetails->event_name }}</td>
                <td><strong class="m-1">Customer Name:</strong> {{ $eventDetails->customer->customer_fname }} {{ $eventDetails->customer->customer_lname }} {{ $eventDetails->customer->suffix ?? '' }}</td>
            </tr>
            <tr>
                <td><strong class="m-1">Event Date:</strong> {{ (\Carbon\Carbon::parse($eventDetails->start_date)->format('M. d-'))}}{{ (\Carbon\Carbon::parse($eventDetails->end_date)->format('d, Y')) }}</td>
            ...
            <tr>
                ...
                <td><strong class="m-1">Contact #:</strong> {{ $eventDetails->customer->contact_no_1 }}{{ ($eventDetails->customer->contact_no_2) ? '/' . $eventDetails->customer->contact_no_2 : '' }}</td>
            </tr>
            <tr>
            ...
            <tr>
                <td><strong class="m-1">Start Time:</strong> {{ \Carbon\Carbon::parse($eventDetails->arrival_time)->format('h:i A') }} ( {{ \Carbon\Carbon::parse($eventDetails->start_date)->format('M. d, Y') }} )</td>
                <td><strong class="m-1">End Time:</strong> {{ \Carbon\Carbon::parse($eventDetails->departure_time)->format('h:i A') }} ( {{ \Carbon\Carbon::parse($eventDetails->end_date)->format('M. d, Y') }} )</td>
            </tr>
        </tbody>
    </table>
    ...
    <div class="col-md-12">
        <table class="table-bordered w-full mt-4">
            <thead>
                <tr>
                ...
                <tr>
                    <td>
                        @foreach($eventDetails->eventVenues ?? [] as $eventVenue)
                            <p class="m-1 t-sm">{!! ($eventVenue->venue->venue_name) ? $eventVenue->qty . 'x '.$eventVenue->venue->venue_name . ' (Good for ' . $eventVenue->venue->capacity . ' Guests) - ₱' . number_format($eventVenue->ratePrice->amount * $eventVenue->qty, 2) : '' !!}</p>
                        @endforeach
                    </td>
                </tr>
            </tbody>
    ...
    <footer style="width: 100%; margin-top: 2rem;">
        <div class="mt-4 row" style="display: flex; width: 100%;">
            <div class="text-center col-md-4" style="flex: 1;">
                ...
                <p class="mb-0 signaroty-name">  {{ $eventDetails->createdBy->name ?? '' }} {{ $eventDetails->createdBy->last_name ?? '' }}  </p>
                <p class="mb-0">{{ $eventDetails->createdBy->position->position_name ?? '' }}</p>
            </div>
            <div class="text-center col-md-4" style="flex: 1;">
                <p class="mb-0">Reviewed By:</p>
                <p class="mb-0 signaroty-name">  {{ $eventDetails->reviewer->name ?? '' }} {{ $eventDetails->reviewer->last_name ?? '' }}  </p>
                <p class="mb-0">{{ $eventDetails->reviewer->position->position_name ?? '' }}</p>
            </div>
            <div class="text-center col-md-4" style="flex: 1;">
                <p class="mb-0">Approved By:</p>
                <p class="mb-0 signaroty-name">  {{ $eventDetails->approver->name ?? '' }} {{ $eventDetails->approver->last_name ?? '' }}  </p>
