<div class="overflow-x-auto" wire:ignore.self>
    <ul class="nav nav-tabs" id="accountingTab" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link active" id="template-tab" data-bs-toggle="tab" data-bs-target="#template-summary"
                type="button" role="tab" aria-controls="template-summary" aria-selected="true">Account
                Template</button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link" id="account-titles-tab" data-bs-toggle="tab" data-bs-target="#account-titles"
                type="button" role="tab" aria-controls="account-titles" aria-selected="false">
    ...
    <div class="tab-content" id="accountingTabContent">
        <div class="container tab-pane fade show active" id="template-summary" role="tabpanel"
            aria-labelledby="template-tab" wire:ignore.self>
            <div class="row">
                <div class="col-md-6">
                    @if (auth()->user()->employee->getModulePermission('Accounting - Template Management') == 1)
                        <a href="/coa-management?action=new"
                            style="text-decoration: none; color: white;"><x-primary-button>+ New
                                Template</x-primary-button></a>
                            ...
                            <tbody>
                                @forelse ($templates as $template)
                                    <tr>
                                        ...
                                        <td>{{ $template->approved_date != null ? $template->approved_date : 'Pending' }}
                                        </td>
                                        <td>
                                            <a
                                                href="{{ route('template.view', ['template' => $template->id, 'action' => 'view']) }}">
        ...
        <div class="container tab-pane fade" id="account-titles" role="tabpanel" aria-labelledby="account-titles-tab"
            wire:ignore.self>
            <div class="row">
                <div class="col-md-6">
                            ...
                            <tbody>
                                @forelse ($accountTitles as $title)
                                    <tr>
                                        ...
                                        <td class="truncate max-w-[150px]" title="{{ $title->description }}"
                                            style="cursor: help">{{ Str::limit($title->description ?? 'N/A', 30) }}
                                        </td>
                                        <td>
                                            <span
                                                @if ($title->is_active == 1) class="badge bg-success" @else class="badge bg-danger" @endif>
        ...
        <div class="container tab-pane fade" id="job-order" role="tabpanel" aria-labelledby="job-order-tab"
            wire:ignore.self>
            <div class="row">
                <div class="col-md-6">
                            ...
                            <tbody>
                                @forelse ($accountTypes as $type)
                                    <tr>
                                        ...
                                        <td class="truncate max-w-[150px]" title="{{ $type->description ?? 'N/A' }}"
                                            style="cursor: help">{{ Str::limit($type->description ?? 'N/A', 30) }}
                                        </td>
                                        <td>
                                            <span
                                                @if ($type->is_active == 1) class="badge bg-success" @else class="badge bg-danger" @endif>
    ...
    <x-modal-card title="Account Title - Create" name="cardModal" wire:ignore.self>
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <x-input label="Account Title" placeholder="Title name ..." wire:model="accountTitle" />
            <x-input label="Account Code" placeholder="Enter account code ..." wire:model="accountCode" />
        </div>
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <x-select label="Parent Title" placeholder="Select parent title ..." :options="$accountTitles" option-value="id"
                :min-items-for-search="0" option-label="account_title" wire:model="parentTitle" />
            <x-select label="Account Type" placeholder="Select account type ..." :options="$accountTypes" option-value="id"
                :min-items-for-search="0" option-label="type_name" wire:model="accountType" />
        </div>
        <div class="grid grid-cols-1 gap-4 mt-2 sm:grid-cols-2">
            <x-select label="Normal Balance" placeholder="Select normal balance ..." :options="['CREDIT', 'DEBIT']"
        ...
        <x-slot name="footer" class="flex justify-between gap-x-4">
            ...
            <div class="flex gap-x-4">
                ...
                <x-primary-button wire:loading.attr="disabled" wire:click="saveAccountTitle"
                    wire:loading.attr="disabled">
                    <span wire:loading wire:target="saveAccountTitle"><i class="spinner-border spinner-border-sm"
    ...
    <x-modal-card title="Account Type - Create" name="cardModalType" wire:ignore.self>
        ...
        <x-input label="Account Type" placeholder="Type name ..." wire:model="typeName" />
        ...
        <x-slot name="footer" class="flex justify-between gap-x-4">
            ...
            <div class="flex gap-x-4">
                ...
                <x-primary-button wire:loading.attr="disabled" wire:click="saveAccountType"
                    wire:loading.attr="disabled">
                    <span wire:loading wire:target="saveAccountType"><i class="spinner-border spinner-border-sm"
                            role="status" aria-hidden="true"></i>&nbsp;Saving...</span>
