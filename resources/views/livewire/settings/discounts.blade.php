            ...
            <div class="row">
                <div class="col-md-6">
                    @if (auth()->user()->employee->getModulePermission('discounts'))
                        <x-primary-button type="button" class="mb-3 btn-sm"
                            onclick="showTab('discount-form', document.querySelector('.nav-link.active'))">+ ADD
                            DISCOUNT</x-primary-button>
                    @endif
                    <x-secondary-button type="button" class="mb-3 btn-sm" wire:click="fetchDiscounts()">
                        <span wire:loading.remove wire:target="fetchDiscounts()">Refresh</span>
                        <span wire:loading wire:target="fetchDiscounts()"><span
                ...
                <div class="col-md-6">
                    <div class="mb-3 input-group">
                        ...
                        <input type="text" class="form-control" id="search-discount" onkeyup="filterDiscounts()">
                    </div>
                </div>
            </div>
            <script>
                    ...
                    <tbody>
                        @forelse ($discounts ?? [] as $discount)
                            <tr>
                                ...
                                <td class="text-end">
                                    {{ $discount->amount == '0.00' ? $discount->percentage . '%' : '₱' . $discount->amount }}
                                </td>
                                ...
                                @if (auth()->user()->employee->getModulePermission('Discounts'))
                                    <td class="text-end">
                                        <a href="#" class="btn btn-sm btn-primary"
                                            onclick="editDiscount({{ json_encode($discount) }})" data-bs-toggle="modal"
                                            data-bs-target="#updateDiscountModal"
                                            wire:click="editDiscount({{ $discount->id }})">Edit</a>
                                        <a href="#" class="btn btn-sm btn-danger"
                                            wire:click="deactivateDiscount({{ $discount->id }})">Disable</a>
                                    </td>
                                @endif
                            </tr>
                        @empty
            ...
            <form wire:submit.prevent="storeDiscount" id="discountForm">
                <div class="mb-3 row">
                    <div class="mb-3 col-md-6">
                        ...
                        <input type="text" class="form-control" id="discount_title-input_add"
                    ...
                    <div class="mb-3 col-md-6">
                        ...
                        <div class="mb-3 input-group">
                            <div class="input-group-text">
                                <input class="mt-0 form-check-input" type="checkbox" id="add_code_checkbox"
                                    wire:model.live="add_code">
                                <label class="form-check-label ms-2" for="add_code_checkbox">Add Code</label>
                            </div>
                            <input type="text" class="form-control" id="discount_code-input"
                                wire:model="discount_code" @if (!$add_code) disabled @endif>
                            @error('discount_code')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="row col-md-12">
                        <div class="mb-3 col-md-4">
                            <label for="discount_title-input_add" class="form-label">Rate / Value<span
                                    style="color: red;">*</span></label>
                            <div class="mb-3 input-group">
                                <input type="text" class="form-control" id="discount_title-input_add"
                                    wire:model="discount_ratevalue">
                                <select name="disc_type" class="form-select form-select-sm"
                                    wire:model.live="discount_ratevalue_type">
                        ...
                        <div class="mb-3 col-md-4">
                            <label for="discount_type-input" class="form-label">Type <span
                                    style="color: red;">*</span></label>
                            <select name="disc_type" class="form-select form-select-sm"
                                wire:model.live="discount_type" @if ($autoapply_discount) disabled @endif>
                        ...
                        <div class="col-md-4 ">
                            <label for="autoapply_checkbox" class="form-label"><span
                                    style="color: rgb(110, 110, 110);">(Optional)</span></label>
                            <div class="mb-3 input-group">
                                <label class="form-check-label ms-2 input-group-text" for="autoapply_checkbox">Auto
                                    Apply</label>
                                <div class="input-group-text">
                                    <input class="form-check-input" type="checkbox" id="autoapply_checkbox"
                                        wire:model.live="autoapply_discount">
                                </div>
                            </div>
                        </div>
                    </div>

                    {{-- <div class="p-2 rounded row" style="background-color: #f8f9fa;">
                          <div class="col-md-2">
                            <input class="mt-0 form-check-input" type="checkbox" id="add_period_checkbox" wire:model.live="add_period">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="discount_description-input_update" class="form-label">Description <span
                                style="color: red;">*</span></label>
                        <textarea class="form-control" id="discount_description-input_update" wire:model="discount_description"
                            rows="3"></textarea>
                        @error('discount_description')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
