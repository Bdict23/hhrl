<div>
            ...
            <div class="row">
                <div class="col-md-6">
                    @if (auth()->user()->employee->getModulePermission('Employee Positions') == 1)
                        <x-primary-button type="button" class="mb-3 btn-sm"
                            onclick="showTab('employee-positions-form', document.querySelector('.nav-link.active'))">+
                            ADD
                ...
                <div class="col-md-6">
                    <div class="mb-3 input-group">
                        ...
                        <input type="text" class="form-control" id="search-position"
                            placeholder="Search Position Name or Description" onkeyup="filterPositions()">
                    </div>
                </div>
            </div>
            <script>
                ...
                <table class="table table-striped table-sm small">
                    <thead class="table-dark sticky-top">
                        <tr>
                            ...
                            <th>DESCRIPTION</th>
                            <th>STATUS</th>
                            <th class="text-end" @if (auth()->user()->employee->getModulePermission('Employee Positions') != 1) style="display: none;" @endif>ACTIONS
                            </th>
                        </tr>
                    </thead>
                        ...
                        @forelse ($positions ?? [] as $position)
                            <tr>
                                ...
                                <td
                                    ...
                                    {{ $position->position_description ?? 'N/A' }}</td>
                                <td>{{ $position->position_status }}</td>
                                @if (auth()->user()->employee->getModulePermission('Employee Positions') == 1)
                                    @if ($position->position_status == 'ACTIVE')
                                        <td class="text-end">
                                            <button type="button" class="btn btn-sm btn-primary"
                                                data-bs-toggle="modal" data-bs-target="#UpdatePosition"
                                                onclick="updatePosition({{ json_encode($position) }})"
                                                wire:click="editPosition({{ $position->id }})">Edit</button>
                                            <a href="#" class="btn btn-sm btn-danger"
                                                wire:click="deactivatePosition({{ $position->id }})">Deactivate</a>
                                        </td>
                                    @else
                                        <td class="text-end">
                                            <button type="button" class="btn btn-sm btn-primary"
                                            ...
                                            <a href="#" class="btn btn-sm btn-success"
                                                wire:click="activatePosition({{ $position->id }})">Activate</a>
                                        </td>
                                    @endif
                                @endif
                            </tr>
    ...
    {{-- Update Modal --}}
    <div class="modal fade" id="UpdatePosition" tabindex="-1" aria-labelledby="updatePositionModal" aria-hidden="true"
        wire:ignore.self>
                    ...
                    <form action="" wire:submit.prevent="updatePosition" id="UpdatePositionForm">
                        <div class="row">
                            <div class="mb-3 col-md-12">
                                ...
                                <input type="text" class="form-control" id="position_name-update-input"
                                    wire:model="position_name_input">
                        ...
                        <div class="mb-3 ">
                            ...
                            <textarea class="form-control" id="position_description-update-input" wire:model="position_description_input"
                                rows="3"></textarea>
    ...
    <div id="employee-positions-form" class="tab-content card" style="display: none;" wire:ignore.self>
        ...
        <div class="card-body">
            ...
            <form wire:submit.prevent="storePosition" id="positionForm">
                @csrf
                <div class="mb-3">
                    <label for="position_name-input" class="form-label">Position Name <span
                            style="color: red;">*</span></label>
                    <input type="text" class="form-control" id="position_name-input"
                        wire:model="position_name_input">
                    @error('position_name_input')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                ...
                <div class="mb-3">
                    <label for="position_description-input" class="form-label">Description <span
                            style="color: red;">*</span></label>
                    <textarea class="form-control" id="position_description-input" wire:model="position_description_input"
                        rows="3"></textarea>
                    @error('position_description_input')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
